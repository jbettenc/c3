...
function Navbar(props: NavbarProps) {
  const { logoOnly = false, applyMaxWidth = true, fixed = true } = props;
  ...
  return (
    <div className="w-full absolute">
      <nav className={`select-none${!logoOnly ? " w-full" : ""}`}>
        <div
          id="nav"
          className={`border-b border-gray-300 bg-white flex flex-col lg:flex-row ${fixed ? "fixed " : ""}z-10${
            !logoOnly ? " w-full" : ""
          }`}
        >
          <div className={`w-full flex flex-col lg:flex-row mx-auto${applyMaxWidth ? " max-w-7xl" : ""}`}>
            <div id="nav-top" className="bg-transparent w-full px-4 lg:px-8">
